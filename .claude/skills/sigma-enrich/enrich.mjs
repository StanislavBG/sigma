...
async function getJSON(url, opts) {
  try { const r = await fetch(url, opts); const t = await r.text();
...
async function gleif(eik) {
  const r = await getJSON(`https://api.gleif.org/api/v1/lei-records?filter[entity.registeredAs]=${encodeURIComponent(eik)}`);
  const rec = r.body?.data?.[0]; if (!rec) return null;
  const a = rec.attributes; const lei = a.lei;
  let parent = null;
  ...
  return { lei, legal_form: a.entity?.legalForm?.id ?? null, status: a.entity?.status ?? null,
    address: a.entity?.legalAddress ?? null, validation: a.registration?.corroborationLevel ?? null,
    parent, raw: a };
}
async function ted(name) {
  ...
  const r = await getJSON('https://api.ted.europa.eu/v3/notices/search', {
    ...
    body: JSON.stringify({ query: `tendering-party-name="${name.replace(/"/g, '')}"`, fields: ['publication-number'], limit: 1 }),
  });
...
async function companybook(eik, doFin) {
  ...
  if (!c.ok || c.body?.error) return { error: c.body?.errorEN || c.body?.error || `HTTP ${c.status}` };
  const co = c.body.company || c.body;
  const out = { legal_form: co.legalForm ?? null, status: co.status ?? null, seat: co.seat ?? null,
    capital: co.capital ?? null, managers: co.managers ?? null, partners: co.partners ?? null,
    nkids: co.nkids ?? null, subsidiaries: Array.isArray(c.body.daughters) ? c.body.daughters.length : null,
    financials: null, raw: co };
  if (doFin) {
...
// ---- target selection ----------------------------------------------------
function num(s) { if (s == null || s === '') return 0; return parseFloat(String(s).replace(/\s/g, '').replace(/\./g, '').replace(',', '.')) || 0; }
function targets(db) {
  ...
  if (typeof flag('--eik') === 'string') {
    const eiks = String(flag('--eik')).split(',').map((s) => s.trim()).filter(Boolean);
  ...
  if (has('--top')) {
    ...
    if (ct > 0)
      return db.prepare(`SELECT eik, name FROM company_totals WHERE eik IS NOT NULL AND eik_valid=1 ORDER BY won_eur DESC LIMIT ?`).all(n);
    ...
    return db.prepare(
      `SELECT b.bulstat AS eik, b.name AS name, SUM(COALESCE(c.amount_eur,0)) won
       ...
       WHERE b.bulstat IS NOT NULL GROUP BY b.bulstat ORDER BY won DESC LIMIT ?`).all(n);
  }
  if (has('--all-bidders')) {
    const lim = Number(flag('--all-bidders')) || 100000;
    return db.prepare(`SELECT bulstat AS eik, name FROM bidders WHERE bulstat IS NOT NULL LIMIT ?`).all(lim);
  }
...
for (const t of list) {
  ...
  if (!/^[0-9]{9,13}$/.test(eik)) { console.error('skip (bad eik):', eik); continue; }
  if (!has('--force')) {
    const prev = getFresh.get(eik);
    if (prev?.enriched_at && (now - Date.parse(prev.enriched_at)) < staleDays * 86400e3) { skipped++; continue; }
  }
  const sources = [];
  const g = await gleif(eik); if (g) sources.push('gleif');
  const td = await ted(t.name); if (td?.notice_count != null) sources.push('ted');
  let cb = null;
  if (!noCB && KEY && cbBudget > 0) {
    const doFin = finBudget > 0;
    cb = await companybook(eik, doFin);
    if (cb && !cb.error && !cb.skipped) { sources.push('companybook'); cbBudget--; if (doFin && cb.financials) finBudget--; }
  }
  const raw = JSON.stringify({ gleif: g?.raw, companybook: cb?.raw, ted: td });
  const row = [eik, t.name ?? null,
    cb?.legal_form ?? null, cb?.status ?? null, J(cb?.seat), J(cb?.capital), J(cb?.managers), J(cb?.partners), J(cb?.nkids), cb?.subsidiaries ?? null, J(cb?.financials),
    g?.lei ?? null, g?.legal_form ?? null, g?.status ?? null, J(g?.address), g?.validation ?? null, g?.parent ?? null,
    td?.notice_count ?? null, JSON.stringify(sources), raw, new Date().toISOString()];
  if (dry) console.log(JSON.stringify({ eik, name: t.name, sources, lei: g?.lei, ted: td?.notice_count, cb: cb?.status ?? cb?.error ?? cb?.skipped }));
  else upsert.run(...row);
  done++;
  console.error(`  ${done}/${list.length} ${eik} [${sources.join(',') || 'none'}]${cb?.error ? ' cb:' + cb.error : ''}`);
  await sleep(250);
}
