...
beforeAll(() => {
  ...
  // Full migration chain — the fixture writes 0005's health-index columns.
...
describe('getQuality — ranking direction', () => {
  ...
  it('direction composes with the rollup pager (page 2 of the flipped order)', async () => {
    const p2 = await getQuality(d1, { grain: 'authority', dir: 'desc', top: 1, rankPage: 2 });
...
describe('getQuality — ranking avg-index range (?rfrom/?rto)', () => {
  ...
  it('drops malformed bounds at the query boundary — non-int / out-of-range never reach SQL', async () => {
    for (const bad of [-5, 101, 3.5, Number.NaN, Number.POSITIVE_INFINITY]) {
      ...
      expect(r.rankingTotal).toBe(2); // unfiltered — the bogus bound was dropped, not clamped
    }
    const str = await getQuality(d1, { grain: 'authority', rankFrom: '10; --' as never });
    expect(str.scope.rankFrom).toBeNull();
    expect(str.rankingTotal).toBe(2);
  });

  it('composes with sort, direction and the pager (rpage over the filtered set)', async () => {
    ...
    expect(r.ranking.map((x) => x.key)).toEqual(['auth:100000002']); // fewest-first, page 2
    expect(r.rankingTotal).toBe(2);
  });
});
