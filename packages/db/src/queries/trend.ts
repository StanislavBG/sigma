// Spending over time — procurement value per period (month, quarter or year) for the /trends chart. Live
...
export async function getSpendingTrend(
  ...
  if (granularity === 'quarter' && rows.length) {
    ...
    for (const r of rows) {
      ...
      const acc = byQuarter.get(period) ?? { period, value_eur: 0, eu_value_eur: 0, contracts: 0 };
      acc.value_eur += r.value_eur;
      acc.eu_value_eur += r.eu_value_eur;
  ...
  const years: TrendYearPlus[] = sortedYears.map((year) => {
