...
describe('getSpendingTrend', () => {
  ...
  it('never computes YoY against a non-adjacent year when a whole year is missing', async () => {
    ...
    const db = {
      prepare(sql: string) {
        return {
          ...
          async all<T>() {
            if (sql.includes('AS quarter') || sql.includes('AS division')) return { results: [] };
            return {
              results: [
                { period: '2020', value_eur: 4000, eu_value_eur: 0, contracts: 40 },
                { period: '2022', value_eur: 5000, eu_value_eur: 0, contracts: 50 },
              ] as T[],
            };
          },
          async first<T>() {
    ...
    expect(years).toEqual([
      { year: '2020', valueEur: 4000, euValueEur: 0, contracts: 40, yoyPct: null, partial: false },
      { year: '2021', valueEur: 0, euValueEur: 0, contracts: 0, yoyPct: -1, partial: false }, // real -100% vs 2020
      { year: '2022', valueEur: 5000, euValueEur: 0, contracts: 50, yoyPct: null, partial: false }, // NOT (5000-4000)/4000
    ]);
  });
      ...
      prepare(sql: string) {
        return {
          ...
          async all<T>() {
            return {
              results: [
                { period: '2022', value_eur: 4000, eu_value_eur: 0, contracts: 40 },
                { period: '2023', value_eur: 1500, eu_value_eur: 0, contracts: 15 },
              ] as T[],
            };
          },
          async first<T>() {
